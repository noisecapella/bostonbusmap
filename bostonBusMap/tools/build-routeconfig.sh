...
cat routes | while read each; do cat "routeConfig$each" | awk 'BEGIN { in_path = 0 } $0 ~ /<body/ { next } $0 ~ /body>/ { next } $0 ~ /<\?xml/ { next } $0 ~ /<path/ { in_path=1; next; } $0 ~ /path>/ { in_path=0; next; } { if (!in_path) { print } }' >> routeconfig_full.xml; done
