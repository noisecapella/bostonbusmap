#!/bin/sh
wget "http://webservices.nextbus.com/service/publicXMLFeed?a=lametro&command=routeList" -O routeList
cat routeList | grep route | awk -F"\"" '{ print $2 }' > routes

for each in `cat routes`; do wget "http://webservices.nextbus.com/service/publicXMLFeed?a=lametro&command=routeConfig&verbose&r=$each" -O routeConfig$each; sleep 10; done

touch routeconfig_full.xml
echo "<?xml version=\"1.0\" encoding=\"utf-8\" ?> " >> routeconfig_full.xml
echo "<body copyright=\"All data copyright Los Angeles Metro 2011.\">" >> routeconfig_full.xml
