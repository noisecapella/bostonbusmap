...
for each in `cat routes`; do wget "http://webservices.nextbus.com/service/publicXMLFeed?a=mbta&command=routeConfig&terse&r=$each" -O routeConfig$each; sleep 20; done
